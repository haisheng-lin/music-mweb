...
// 获取除了 state 本身自外的其他函数参数
type GetExtraParams<U, T> = U extends (value: T, ...args: infer P) => void
  ? P
  : never;
 ...
 * @param {K} methods 需要 hook 化的方法
 * @returns {[T, ReturnMethods<K, T>]}
 */
const useMethods = <T, K extends FilterMethods<K, T>>(
  initState: T,
  methods: K
): [T, ReturnMethods<K, T>] => {
